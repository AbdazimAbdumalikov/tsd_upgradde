              ...
              children: [
                ...
                BlocListener<RefundContainerCubit, RefundContainerState>(
                  ...
                  child: CustomButton(
                    height: 44,
                    onClick: () async {
                      // Проверка разрешений перед вызовом refundContainer
                      var locationPermission = await Geolocator.checkPermission();
                      if (locationPermission == LocationPermission.denied) {
                        await Geolocator.requestPermission();
                      }

                      // Проверка включенности службы местоположения
                      var locationServiceEnabled = await Geolocator.isLocationServiceEnabled();
                      if (locationServiceEnabled) {
                        // Создание списка имен из containers
                        ...
                        // Служба местоположения отключена
                        // Обработка ошибки или предупреждение
                        print('Служба местоположения отключена');
                      }
                    },
                    body: const Text(
                      'Завершить',
